...
const FootwearPostDetails = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const postResponse = await axios.get(`${baseURL}/footwear/footwearpostsbyid/${id}`);
      ...
      console.error("Error fetching footwear post details:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendNumber = async () => {
    ...
    try {
      await axios.post(`${baseURL}/api/sendPhoneNumberFootwear`, {
        postId: post.id,
        phoneNumber,
      });
  ...
  const handleWhatsAppContact = () => {
    if (post && post.phone_number) {
      const whatsappUrl = `whatsapp://send?phone=${post.phone_number}&text=Hi, I'm interested in your footwear item listing.`;
  ...
  const imagesArray = Array.isArray(post.images)
    ...
    : typeof post.images === "string"
        ? (post.images.startsWith('[') ? JSON.parse(post.images) : post.images.split(","))
        : [];
        ...
        <View style={styles.contentPadding}>
          ...
          <View style={styles.titleSection}>
            <Text style={[styles.mainTitle, isDark ? styles.darkTitle : styles.lightTitle]}>
              {post.description || "Footwear Item"}
            </Text>
          ...
          <View style={[styles.detailsCard, isDark ? styles.darkCard : styles.lightCard]}>
            <View style={styles.detailsGrid}>
              ...
              <View style={styles.gridItem}>
                <View style={styles.gridLabelRow}>
                  <Ionicons name="expand-outline" size={16} color={isDark ? "#aaa" : "#666"} />
                  <Text style={[styles.gridLabel, isDark ? styles.darkSubText : styles.lightSubText]}>Size</Text>
                </View>
                <Text style={[styles.gridValue, isDark ? styles.darkText : styles.lightText]}>{post.size || "N/A"}</Text>
              </View>
...
export default FootwearPostDetails;
