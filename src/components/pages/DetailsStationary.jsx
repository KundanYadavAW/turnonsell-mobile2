...
const DetailsStationary = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const postResponse = await axios.get(`${baseURL}/stationary/stationarypostsbyid/${id}`);
      setPost(postResponse.data);

      if (postResponse.data.createdAt) {
        const postDate = new Date(postResponse.data.createdAt);
        const now = new Date();
      ...
      // Fetch Dropdowns
      const collegesResponse = await axios.get(`${baseURL}/dropdown/colleges`);
      setColleges(collegesResponse.data);

      const degreesResponse = await axios.get(`${baseURL}/dropdown/degrees`);
      setDegrees(degreesResponse.data);

    } catch (error) {
      console.error("Error fetching stationary post details:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendNumber = async () => {
    ...
    try {
      await axios.post(`${baseURL}/stationary/sendPhoneNumberStationary`, {
        postId: post.id,
        phoneNumber,
      });
  ...
  const handleWhatsAppContact = () => {
    if (post && post.phone_number) {
      const whatsappUrl = `whatsapp://send?phone=${post.phone_number}&text=Hi, I'm interested in your ${post.stationary_name} listing.`;
  ...
  const college = colleges.find(c => c.id == post.college_id);
  const collegeName = college ? college.name : "Unknown College";

  const degree = degrees.find(d => d.id == post.degree_id);
  const degreeName = degree ? degree.name : "Unknown Degree";
        ...
        {imagesArray.length > 0 ? (
          <View style={styles.imageContainer}>
            ...
            <View style={styles.priceOverlay}>
              ...
              <Text style={styles.overlayPriceText}>₹{post.price}</Text>
            </View>
          </View>
        ) : (
            ...
            <View style={styles.detailsGrid}>
              ...
              <View style={styles.gridItem}>
                <View style={styles.gridLabelRow}>
                  <Ionicons name="school-outline" size={16} color={isDark ? "#aaa" : "#666"} />
                  <Text style={[styles.gridLabel, isDark ? styles.darkSubText : styles.lightSubText]}>College</Text>
                </View>
                <Text style={[styles.gridValue, isDark ? styles.darkText : styles.lightText]}>{collegeName}</Text>
              </View>
              
              <View style={styles.gridItem}>
                <View style={styles.gridLabelRow}>
                  <Ionicons name="ribbon-outline" size={16} color={isDark ? "#aaa" : "#666"} />
                  <Text style={[styles.gridLabel, isDark ? styles.darkSubText : styles.lightSubText]}>Degree</Text>
                </View>
                <Text style={[styles.gridValue, isDark ? styles.darkText : styles.lightText]}>{degreeName}</Text>
              </View>

              <View style={styles.gridItem}>
                <View style={styles.gridLabelRow}>
                  <Ionicons name="calendar-outline" size={16} color={isDark ? "#aaa" : "#666"} />
                  <Text style={[styles.gridLabel, isDark ? styles.darkSubText : styles.lightSubText]}>Year</Text>
                </View>
                <Text style={[styles.gridValue, isDark ? styles.darkText : styles.lightText]}>{post.year}</Text>
              </View>
