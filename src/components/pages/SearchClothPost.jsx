...
//       setColleges(collegesRes.data.map(c => ({ label: c.name, value: c.name }))); // Using name as value for filtering
//       setGenders(gendersRes.data.map(g => ({ label: g.name, value: g.id }))); // Need ID for dependent fetches, will map name for filtering
//     } catch (err) {
...
//       setSubcategories(response.data.map(s => ({ label: s.name, value: s.name }))); // Using name for filtering
//     } catch (error) {
...
//     try {
//       setLoading(true);
//       const response = await axios.get(`${baseURL}/cloth/clothposts`);
//       // Shuffle posts
//       const shuffled = response.data.sort(() => Math.random() - 0.5).slice(0, 100);
//       setPosts(shuffled);
//       setFilteredPosts(shuffled);
//     } catch (err) {
//       setError("Failed to load fashion posts");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//         onPress={() => navigation.navigate("ClothPostDetails", { id: item.id })}
//       >
...
//               <Ionicons name="shirt-outline" size={40} color={isDark ? "#aaa" : "#555"} />
//             </View>
//           )}
...
//         <View style={styles.centerContainer}>
//           <Ionicons name="shirt-outline" size={64} color={isDark ? "#555" : "#ccc"} />
...
//     bottom: 8,
//     right: 8,
//     backgroundColor: "#2196F3", // Blue for fashion
//     paddingHorizontal: 8,
//     paddingVertical: 4,
//     borderRadius: 12,
//   },
...
// export default SearchClothPost;
      ...
      >
        ...
        <View style={[styles.modalSheet, isDark ? styles.darkModal : styles.lightModal]}>
          ...
          {value && (
            ...
            >
              <Ionicons name="close-circle-outline" size={15} color="#2196F3" />
...
// ─── Main SearchClothPost Component ──────────────────────────────────────────
const SearchClothPost = () => {
  const [posts, setPosts] = useState([]);
  const [filteredPosts, setFilteredPosts] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  // ── Handler: Gender ────────────────────────────────────────────────────────
  // Gender is special: changing it resets category + subcategory
  // and triggers a dependent fetch for categories
  ...
  // ── Handler: Category ──────────────────────────────────────────────────────
  // Category is special: changing it resets subcategory
  // and triggers a dependent fetch for subcategories
...
const styles = StyleSheet.create({
  ...
  clearText: { color: "#2196F3", fontWeight: "600", fontSize: 15 },
  ...
  lightClearBtn: { backgroundColor: "#E3F2FD" },
  darkClearBtn: { backgroundColor: "#1A2744" },
  clearSelectionText: { color: "#2196F3", fontWeight: "600", fontSize: 14 },
  ...
  lightSelectedOption: { backgroundColor: "#E3F2FD" },
  darkSelectedOption: { backgroundColor: "#1A2744" },
  optionText: { flex: 1, fontSize: 14, marginRight: 8 },
  selectedOptionText: { color: "#2196F3", fontWeight: "600" },
  ...
  priceTag: { position: "absolute", bottom: 8, right: 8, backgroundColor: "#2196F3", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12 },
